...
def main():
    ...
    if options:
        ...
        output_bytes = b.create_ota_image(**options)
...
class BootImage:
    ...
    def create_ota_image(self, **kwargs):
    ...
    def generate_opt(self, binary, image_type):
        # Find the opt bitfield definitions for our selected image type.

        image_opt_offsets = self.opt_offsets[image_type]
        return bitfields_from_dict(input_dict, image_opt_offsets, length=4)
    ...
    def sign_byte_array_dll(self, B, key, auth_algo=0):
        ...
        #BinStr = B.decode('iso-8859-1')
        signature = GetRSASignature(log_file, byte2string(B), key.filename, key.pass_file, DLLHandle)
        # print(BinStr.encode('iso-8859-1')[0:8])
        # print(byte2string(B).encode('iso-8859-1')[0:8])
