...
def parse_config_file(config, log_file):
    ...
    # OPT_SEC_POL
    # OPT_SEC_POL.AUTH_ENF_ECC
    if config.has_option(section_name, 'OPT_SEC_POL.AUTH_ENF_ECC'):
        opt_sec_pol_auth_enf_ecc = int(config.get(section_name, 'OPT_SEC_POL.AUTH_ENF_ECC'),16)
    ...
    # OPT_SEC_POL.ENC_ENFORCE
    if config.has_option(section_name, 'OPT_SEC_POL.ENC_ENFORCE'):
        opt_sec_pol_enc_force = int(config.get(section_name, 'OPT_SEC_POL.ENC_ENFORCE'),16)
        opt_sec_pol_enc_force = opt_sec_pol_enc_force << 26
    else:
        print_and_log(log_file, "OPT_SEC_POL.ENC_ENFORCE: Security Policy Not Provided \n")
        return None, None

    # OPT_SEC_POL.AUTH_ENFORCE
    if config.has_option(section_name, 'OPT_SEC_POL.AUTH_ENFORCE'):
        opt_sec_pol_auth_enforce = int(config.get(section_name, 'OPT_SEC_POL.ENC_ENFORCE'),16)
        opt_sec_pol_auth_enforce = opt_sec_pol_auth_enforce << 23
    else:
        print_and_log(log_file, "OPT_SEC_POL.AUTH_ENFORCE: Security Policy Not Provided \n")
        return None, None

    # OPT_SEC_POL.WRAP_MODE
    if config.has_option(section_name, 'OPT_SEC_POL.WRAP_MODE'):
        opt_sec_pol_wrap_mode = int(config.get(section_name, 'OPT_SEC_POL.WRAP_MODE'),16)
        opt_sec_pol_wrap_mode = opt_sec_pol_wrap_mode << 19
    else:
        print_and_log(log_file, "OPT_SEC_POL.WRAP_MODE: Security Policy Not Provided \n")
        return None, None

    # OPT_SEC_POL.SBL_LOG_EN
    if config.has_option(section_name, 'OPT_SEC_POL.SBL_LOG_EN'):
        opt_sec_pol_sbl_log_en = int(config.get(section_name, 'OPT_SEC_POL.SBL_LOG_EN'),16)
        opt_sec_pol_sbl_log_en = opt_sec_pol_sbl_log_en << 18
    else:
        print_and_log(log_file, "OPT_SEC_POL.SBL_LOG_EN: Security Policy Not Provided \n")
        return None, None
...
def main():
    ...
    if (data_dict != None):
        ...
        # 0x00001840: Hole in reg mapping
        copyBytes(oemAsset_bin, fill_data.to_bytes(4, byteorder="little"), 64, 4)
        #print('Pad: %s' % hex(int('0x1800', 16) + 64))
